...
function_registry <- function() {
  ...
  cat("2. Asignar nombres a elementos de un vector\n")
  ...
  cat("4. Longitud y tipo de vector\n")
  ...
  cat("13. Corchetes [] para subconjuntos\n")
  ...
  cat("14. Convertir columnas character a factores MASIVAMENTE\n")
  cat("    * Plantilla:\n")
  ...
  cat("15. select() - Seleccionar/Eliminar columnas\n")
  ...
  cat("16. Helpers de select() para busqueda inteligente\n")
  ...
  cat("19. filter() - Filtrar filas por condiciones\n")
  ...
  cat("20. subset() - Alternativa R Base a filter + select\n")
  cat("    * Plantilla:\n")
  cat("      df_sub <- subset(df,\n")
  cat("        subset = especie == 'setosa' & num > 10,\n")
  cat("        select = c(col1, col2))\n\n")

  cat("21. slice() - Seleccionar filas por posicion\n")
  ...
  cat("23. count() - Contar frecuencias\n")
  cat("    * Plantilla: df %>% count(columna_categorica)\n")
  cat("    * Plantilla: df %>% count(col1, col2)  # combinaciones\n\n")

  cat("----------------------------------------------------------------------\n")
  cat(" GRUPO 5: DPLYR - CREAR Y MODIFICAR COLUMNAS\n")
  cat("----------------------------------------------------------------------\n\n")

  cat("24. mutate() - Crear/modificar columnas\n")
  ...
  cat("25. group_by() + summarise() - Resumen por grupos\n")
  cat("    * Plantilla:\n")
  cat("      df %>%\n")
  ...
  cat("26. summarise() con across() y list() - Varias columnas a la vez\n")
  cat("    * Plantilla:\n")
  cat("      df %>%\n")
  cat("        group_by(cat) %>%\n")
  ...
  cat("27. summarise() con across() y where() - Todas las numericas\n")
  cat("    * Plantilla:\n")
  cat("      df %>%\n")
  cat("        group_by(cat) %>%\n")
  cat("        summarise(across(\n")
  ...
  cat("31. rowSums() - Sumar por fila (R Base)\n")
  cat("    * Plantilla:\n")
  cat("      df %>% mutate(total = rowSums(select(., starts_with('spp')), na.rm = TRUE))\n\n")

  cat("32. rowwise() + c_across() - CUALQUIER operacion por fila\n")
  cat("    * Plantilla:\n")
  cat("      df %>%\n")
  cat("        rowwise() %>%\n")
  cat("        mutate(\n")
  ...
  cat("44. pivot_wider() - De largo a ancho\n")
  cat("    * Plantilla:\n")
  ...
  cat("48. drop_na() - Eliminar filas con NA\n")
  ...
  cat("49. replace_na() - Reemplazar NAs\n")
  ...
  cat("50. na.rm = TRUE - Ignorar NAs en funciones\n")
  cat("    * Plantilla: mean(v, na.rm = TRUE), sum(v, na.rm = TRUE)\n\n")

  cat("51. Reemplazo directo de celdas\n")
  ...
  cat("52. if() {} else {} - Condicional escalar (R Base)\n")
  cat("    * Plantilla:\n")
  cat("      if (x > 0) {\n")
  cat("        print('Positivo')\n")
  cat("      } else if (x < 0) {\n")
  ...
  cat("58. Sintaxis basica de funciones\n")
  cat("    * Plantilla:\n")
  ...
  cat("59. Funcion sin argumentos\n")
  cat("    * Plantilla:\n")
  cat("      mi_funcion <- function() {\n")
  cat("        runif(1, min = 1, max = 10)\n")
  cat("      }\n\n")

  cat("60. Funcion con salida de texto\n")
  cat("    * Plantilla:\n")
  ...
  cat("61. Funcion que acepta vectores\n")
  cat("    * Plantilla:\n")
  cat("      prom <- function(x) {\n")
  cat("        s <- sum(x)\n")
  cat("        n <- length(x)\n")
  ...
  cat("62. Funcion que devuelve info de columnas de un df\n")
  cat("    * Plantilla:\n")
  ...
  cat("----------------------------------------------------------------------\n")
  cat(" GRUPO 13: LOOPS (FOR Y WHILE)\n")
  cat("----------------------------------------------------------------------\n\n")

  cat("63. for loop - Estructura basica\n")
  cat("    * Plantilla:\n")
  cat("      for (i in 1:10) {\n")
  cat("        print(i)\n")
  cat("      }\n\n")

  cat("64. for loop con vector pre-allocated\n")
  cat("    * Plantilla:\n")
  cat("      resultados <- numeric(m)\n")
  cat("      for (i in 1:m) {\n")
  cat("        resultados[i] <- i + (i - 1)\n")
  cat("      }\n\n")

  cat("65. for loop con cat() para progreso\n")
  cat("    * Plantilla:\n")
  cat("      for (i in seq(-5, 5)) {\n")
  cat("        cat(i, 'al cuadrado es', i^2, '\\n')\n")
  cat("      }\n\n")

  cat("66. for loop con condicion interna (break)\n")
  cat("    * Plantilla:\n")
  cat("      for (i in 1:100) {\n")
  cat("        if (resultados[i-1] < umbral) { break }\n")
  cat("      }\n\n")

  cat("67. for loop sobre nombres de un vector\n")
  cat("    * Plantilla:\n")
  ...
  cat("68. while loop - Mientras condicion se cumpla\n")
  cat("    * Plantilla:\n")
  cat("      contador <- 0\n")
  ...
  cat("87. Equivalencias R Base para stringr\n")
  ...
  cat("88. Plantilla maestra de ggplot2\n")
  cat("    ggplot(data = df) +\n")
  ...
  cat("89. Geometrias (geoms) mas usadas\n")
  ...
  cat("94. Ordenamiento de factores en graficos (forcats)\n")
  ...
  cat("95. Pie chart (grafico de pastel)\n")
  cat("    * Plantilla:\n")
  cat("      df %>%\n")
  cat("        count(categoria) %>%\n")
  ...
  cat("96. Proporciones en barras\n")
  cat("    * position = 'fill'  # escala de 0 a 1 (proporcion)\n")
  cat("    * position = 'dodge' # barras lado a lado\n\n")

  cat("----------------------------------------------------------------------\n")
  cat(" GRUPO 17: FUNCIONES ADICIONALES (del Script Compartido)\n")
  cat("----------------------------------------------------------------------\n\n")

  cat("97. setdiff() - Diferencia entre conjuntos\n")
  cat("    * Plantilla: setdiff(vector_a, vector_b)\n")
  cat("    * Descripcion: Devuelve elementos de A que NO estan en B.\n")
  cat("    * Util para encontrar elementos exclusivos de un grupo.\n\n")

  cat("98. setNames() - Asignar nombres al crear un objeto\n")
  cat("    * Plantilla: v <- setNames(c(1, 2, 3), c('a', 'b', 'c'))\n")
  cat("    * Descripcion: Crea un vector con nombres en un solo paso.\n\n")

  cat("99. if_all() / if_any() - Filtrar filas segun multiples columnas\n")
  cat("    * Plantilla: df %>% filter(if_all(everything(), ~ !is.na(.x)))\n")
  cat("    * Plantilla: df %>% filter(if_any(starts_with('spp'), ~ .x > 0))\n")
  cat("    * Descripcion: if_all() exige que TODAS cumplan; if_any() que AL MENOS UNA.\n\n")

  cat("100. anyNA() - Verificar si existe algun NA\n")
  cat("     * Plantilla: anyNA(df)  # igual a any(is.na(df)) pero mas corto\n")
  cat("     * Plantilla: anyNA(df$col)\n\n")

  cat("101. paste0() - Concatenar strings sin separador\n")
  cat("     * Plantilla: paste0('Hola', ' ', 'Mundo')  # 'Hola Mundo'\n")
  cat("     * Equivale a paste(..., sep = '')\n\n")

  cat("102. do.call(rbind, ...) - Combinar lista de data.frames\n")
  cat("     * Plantilla: df <- as.data.frame(do.call(rbind, mi_lista))\n")
  cat("     * Descripcion: Une multiples df/vectores de una lista fila por fila.\n\n")

  cat("103. str_remove() / str_remove_all() - Eliminar patrones (stringr)\n")
  cat("     * Plantilla: str_remove(string, pattern)  # elimina el primer match\n")
  cat("     * Plantilla: str_remove_all(string, pattern)  # elimina todos\n")
  cat("     * Equivale a: str_replace(string, pattern, '')\n\n")

  cat("104. writeLines() - Mostrar vector elemento por linea\n")
  cat("     * Plantilla: writeLines(vector_de_texto)\n")
  cat("     * Descripcion: Muestra cada elemento en una linea separada.\n")
  cat("     * Util para ver resultados de str_detect() de forma legible.\n\n")

  cat("105. nchar() - Longitud de string (R Base)\n")
  cat("     * Plantilla: nchar('Hola')  # 4\n")
  cat("     * Equivalente R Base de str_length().\n\n")

  cat("106. gregexpr() - Posiciones de todos los matches (R Base)\n")
  cat("     * Plantilla: gregexpr('A', 'ATCGATCG')\n")
  cat("     * Descripcion: Devuelve las posiciones de TODAS las coincidencias.\n")
  cat("     * Acceder con [[1]] para obtener el vector de posiciones.\n\n")

  cat("107. exp() / log() - Transformaciones matematicas\n")
  cat("     * Plantilla: exp(x)  # e^x (inverso de log natural)\n")
  cat("     * Plantilla: log(x)  # logaritmo natural\n")
  cat("     * Descripcion: Usados para transformar datos (log-normal, etc).\n\n")

  cat("108. View() - Inspeccion visual en RStudio\n")
  cat("     * Plantilla: View(df)\n")
  cat("     * Descripcion: Abre el dataframe en una pestana interactiva.\n")
  cat("     * Util para explorar datos rapidamente sin imprimir en consola.\n\n")

  cat("109. group_by() + mutate() - Estadisticos por grupo SIN colapsar\n")
  cat("     * Plantilla:\n")
  cat("       df %>%\n")
  cat("         group_by(categoria) %>%\n")
  cat("         mutate(promedio_grupo = mean(num, na.rm = TRUE))\n")
  cat("     * Descripcion: A diferencia de summarise(), mutate() anade el\n")
...
menu <- function() {
  ...
  cat("============================================\n\n")
  cat("  function_registry()     - Guia completa (109 temas)\n")
